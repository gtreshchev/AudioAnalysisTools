...
UOnsetDetection* UOnsetDetection::CreateOnsetDetection()
{
...
void UOnsetDetection::DetectOnsetFromEnvelope(FOnsetDetectionStruct OnsetDetectionInfo)
{
...
bool UOnsetDetection::DetectOnsetFromEnvelope_Internal(FOnsetDetectionStruct OnsetDetectionInfo)
{
	if (OnsetDetectionInfo.AnalysedEnvelopeData.EnvelopeTimeData.Num() <= 0)
	{
		OnFinished_Internal(DetectedOnsetArray, EnvelopeTimeDataError);
		return false;
	}

	if (OnsetDetectionInfo.BufferDetectionLength < 2)
	{
		OnFinished_Internal(DetectedOnsetArray, BufferDetectionLengthError);
		return false;
	}
	...
	if (OnsetDetectionInfo.DetectionStepUnitFeature.bUseStepUnit && OnsetDetectionInfo.DetectionStepUnitFeature.
		WaitingTime <= 0.0f)
	{
		OnFinished_Internal(DetectedOnsetArray, StepWaitingTimeError);
		return false;
	}
	...
	if (OnsetDetectionInfo.DetectionTimeoutFeature.bUseTimeout && OnsetDetectionInfo.DetectionTimeoutFeature.WaitingTime
		<= 0.0f)
	{
		OnFinished_Internal(DetectedOnsetArray, TimeoutWaitingTimeError);
		return false;
	}
	...
	{
		OnFinished_Internal(DetectedOnsetArray, WasNotDetected);
		return false;
	}

	OnFinished_Internal(DetectedOnsetArray, SuccessfulDetection);
	DetectedOnsetArray.Empty();
	return true;
}

void UOnsetDetection::CheckOnsetDetection(bool IgnoreAdd, float PlaybackTime,
...
void UOnsetDetection::OnsetDetected(float DetectedTime, float OnsetMultiplier)
{
	DetectedOnsetArray.Add(DetectedTime * OnsetMultiplier);
}

bool UOnsetDetection::DetectOnset(bool IgnoreAdd, float PlaybackTime)
{
...
void UOnsetDetection::ShiftElementsDown(float ValueToSet)
{
...
void UOnsetDetection::UpdateThreshold(float ThresholdExcessAverageDivider, float ThresholdUpdatingDivider)
{
	float TempSum = 0;
...
void UOnsetDetection::DecreaseThreshold(float ThresholdDecreasingDivider)
{
	OnsetThreshold = OnsetThreshold - (OnsetThreshold / ThresholdDecreasingDivider);
}

void UOnsetDetection::OnFinished_Internal(const TArray<float>& InDetectedOnsetArray,
